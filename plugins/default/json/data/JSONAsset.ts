...
interface JSONAssetPub {
  text: string;
  draft: string;
  revisionId: number;
}

export default class JSONAsset extends SupCore.Data.Base.Asset {
  ...
  constructor(id: string, pub: JSONAssetPub, server: ProjectServer) {
    super(id, pub, JSONAsset.schema, server);
  }
  ...
  load(assetPath: string) {
    let pub: JSONAssetPub;
    fs.readFile(path.join(assetPath, "data.json"), { encoding: "utf8" }, (err, text) => {
      fs.readFile(path.join(assetPath, "draft.json"), { encoding: "utf8" }, (err, draft) => {
  ...
  save(assetPath: string, callback: (err: Error) => any) {
    fs.writeFile(path.join(assetPath, "data.json"), this.pub.text, { encoding: "utf8" }, (err) => {
      if (err != null) { callback(err); return; }

      if (this.hasDraft) {
        fs.writeFile(path.join(assetPath, "draft.json"), this.pub.draft, { encoding: "utf8" }, callback);
      } else {
        fs.unlink(path.join(assetPath, "draft.json"), (err) => {
